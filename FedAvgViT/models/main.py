...
def main():
    ...
    for i in range(start_round, num_rounds):
        ...
        if (i + 1) == num_rounds * 0.05 or (i + 1) == num_rounds * 0.25 or (i + 1) == num_rounds * 0.5 or (i + 1) == num_rounds * 0.75:
            where_saved = server.save_model(i+1, os.path.join(ckpt_path, 'round:' + str(i+1) + '_' + job_name + '_' + current_time + '.ckpt'), None)
        else:
            where_saved = server.save_model(i + 1, os.path.join(ckpt_path, ckpt_name), None)
